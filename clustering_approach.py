...
def isValid(arguments):
    '''
    Checks to make sure the input arguments are valid
    '''
    if os.path.exists(arguments[0]):
        return True
    else:
        print 'Incorrect usage of clustering_approach.py, correct usage:\n' +\
                '\t python clustering_approach.py <dir_with_train_or_pickle>'
...
def main(argv):
    '''
    Handles calling the appropriate functions following the schema
    of the algorithm as described in the research paper.
    '''
    have_pickle = True # Bool so that easy run with no parameters
    if (have_pickle):
