...
def numbers(query,sentences):
    ...
    if len(all_nums) >5:
        ...
        top_5 = sorted(all_nums,reverse=True)[:5]
    else:
        top_5 = all_nums
...
def create_file(dir1,dir2):
    ...
    for ff in files:
        ...
        if len(top_5) == 5:
            ...
            top_5 = list(top_5)
            top_5.append(avg)
            top_5.append(classification)
            top_5.append(ff)
            toreturn.append(top_5)
...
def get_whole_array():
    ...
    for i in range(len(dir1_lst)):
        temp = create_file(dir1_lst[i],dir2_lst[i])
        towrite.extend(temp)
        
    wf = open("irrelevancy_arff_type_n5.txt",'w')
    for i in towrite:
        i = i[:-1]
        i = map(str,i)
        i = ",".join(i)
